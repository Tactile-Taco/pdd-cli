#!/usr/bin/env python3
"""PDD Validation Engine — three mandatory layers (ported from pdd-registry).
...
Generalizations over the pdd-registry engine (all backwards-compatible — bundles
without the new manifest fields validate exactly as before):
  * --evidence-dir <dir>   where validation-results.json lands (default cwd/evidence)
  * capability-manifest.yaml capabilities.dependencies declares the O-layer policy
    {allowlist, forbidden_modules, forbidden_calls, forbidden_attrs}; absent -> the
    engine's built-in defaults (stdlib-only allowlist, registry's forbidden sets)
  * candidate-manifest.json structural_error_kinds lists the S-002 error-envelope
    markers (default ["invalid_request"] = pdd-registry behavior)
  * entry_module may name a package dir (impl/<module>/); the candidate digest then
    covers the whole package tree, not a single .py file
...
EVIDENCE_ROOT = Path(os.environ.get("PDD_EVIDENCE_DIR", "evidence"))
...
# Approved runtime dependency allowlist (O-003): stdlib only — the DEFAULT when
# the bundle's capability manifest declares no dependencies.allowlist.
...
DEFAULT_FORBIDDEN_CALL_NAMES = {"open", "eval", "exec", "compile", "__import__", "input"}
DEFAULT_FORBIDDEN_ATTRS = {"system", "popen", "call", "Popen", "run", "connect", "sendto",
                           "sendall", "write_text", "write_bytes", "mkdir", "unlink", "remove"}
DEFAULT_FORBIDDEN_MODULES = {"os", "subprocess", "socket", "urllib", "requests", "pathlib",
...
def candidate_digest(impl: Path, entry_module: str) -> str:
    """Digest of the artifact the validation attests: the entry .py file, or the
    whole package dir when entry_module names a package."""
    target = impl / entry_module
    if target.is_dir():
        return dir_digest(target)
...
def _o_policy(bundle: Path) -> tuple[set, set, set, set]:
    """O-layer policy from capability-manifest.yaml capabilities.dependencies;
    falls back to the engine defaults when absent (backwards compatible)."""
    allowed = set(DEFAULT_ALLOWED_IMPORTS)
    forbidden_modules = set(DEFAULT_FORBIDDEN_MODULES)
    forbidden_calls = set(DEFAULT_FORBIDDEN_CALL_NAMES)
    forbidden_attrs = set(DEFAULT_FORBIDDEN_ATTRS)
    try:
        cap = load_yaml(bundle / "capability-manifest.yaml") or {}
        deps = (cap.get("capabilities") or {}).get("dependencies") or {}
    except Exception:  # noqa: BLE001
        deps = {}
    if deps.get("allowlist"):
        allowed = set(deps["allowlist"])
    if deps.get("forbidden_modules"):
        forbidden_modules = set(deps["forbidden_modules"])
    if deps.get("forbidden_calls"):
        forbidden_calls = set(deps["forbidden_calls"])
    if deps.get("forbidden_attrs"):
        forbidden_attrs = set(deps["forbidden_attrs"])
    return allowed, forbidden_modules, forbidden_calls, forbidden_attrs


def layer_structural(bundle: Path, impl: Path, manifest: dict) -> list[dict]:
    results = []
    impl_files = [p for p in impl.rglob("*.py")
    ...
    # static check: the bundle-declared error kinds are referenced by the
    # candidate source. Default kinds preserve the pdd-registry behavior.
    src = "\n".join(f.read_text() for f in impl_files)
    kinds = manifest.get("structural_error_kinds") or ["invalid_request"]
    results.append({"invariant_id": "S-002", "layer": "structural",
                    "outcome": "pass" if any(k in src for k in kinds) else "fail",
                    "evidence": f"error-kind markers {kinds} referenced in candidate source"})
...
def layer_operational_static(bundle: Path, impl: Path, entry_module: str) -> list[dict]:
    ...
    allowed_imports, forbidden_modules, forbidden_calls, forbidden_attrs = _o_policy(bundle)
    results = []
    impl_files = [p for p in impl.rglob("*.py")
                  if "tests" not in p.parts and p.name != "conftest.py"]
    ...
    # For package candidates the package name and its subpackages are local
    # modules (relative imports), never third-party.
    local_stems = {p.stem for p in impl_files}
    if (impl / entry_module).is_dir():
        local_stems.add(entry_module)
        local_stems |= {p.name for p in (impl / entry_module).iterdir() if p.is_dir()}
    imports = set()
    for node in ast.walk(tree):
...
def mutation_sanity(impl: Path, testdir: Path, pbt_runs: int, manifest: dict) -> dict:
    ...
    # The mutation target is the file containing the pattern: the entry .py
    # for single-file candidates, or exactly one module under the entry
    # package for package candidates (the pattern must match exactly one file
    # or the definition is ambiguous — fail-closed).
    single = impl / f"{entry_module}.py"
    if single.exists():
        targets = [single]
    elif (impl / entry_module).is_dir():
        targets = [p for p in (impl / entry_module).rglob("*.py")
                   if mutant_def["find"] in p.read_text()]
    else:
        return {"invariant_id": mutant_def.get("invariant", "B-001"), "layer": "behavioral",
                "outcome": "mutation-suspect",
                "evidence": f"entry module {entry_module!r} not found for mutation"}
    if len(targets) != 1:
        return {"invariant_id": mutant_def.get("invariant", "B-001"), "layer": "behavioral",
                "outcome": "mutation-suspect",
                "evidence": f"mutation pattern must match exactly one file, matched {len(targets)}"}
    src_file = targets[0]
    ...
    rel = src_file.relative_to(impl)
    with tempfile.TemporaryDirectory() as td:
        mutant_dir = Path(td) / "mutant"
        shutil.copytree(impl, mutant_dir)
        (mutant_dir / rel).write_text(mutant_src)
        proc = subprocess.run(
...
def layer_operational_dynamic(bundle: Path, impl: Path, sandbox: bool, pbt_runs: int,
    ...
    bench_code = (
        "import contextlib, io, json, statistics, sys, time\n"
        "sys.path.insert(0, '.')\n"
        f"from {entry_module} import {entry_class}\n"
        f"reg = {bench_ctor}\n"
        "lat = []\n"
...
def _main(argv: list[str]) -> int:
    if len(argv) < 3:
        print("usage: pdd workflow validate <bundle-dir> --impl <impl-dir> [--sandbox] [--pbt-runs N]")
        return 2
    ...
    if "--evidence-dir" in argv:
        ...
        if idx + 1 >= len(argv):
            print("--evidence-dir requires a value")
            return 2
        EVIDENCE_ROOT = Path(argv[idx + 1])
    ...
    results = (layer_structural(bundle, impl, manifest)
               + layer_operational_static(bundle, impl, entry_module)   # static scan BEFORE any execution
    ...
    out = {
        ...
        "candidate_digest": candidate_digest(impl, entry_module),
        "validators": [
    ...
    out_path = EVIDENCE_ROOT / name / "validation"
